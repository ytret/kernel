...
kerr_t file_open_path(const vpath_t *path, file_t *file) {
    ...
    vnode_t *node;
    const kerr_t err = vnode_resolve_path(path, &node);
    if (err != KERR_NONE) {
        LOG_ERROR("failed to resolve path, error %d (%s)", err, kerr_str(err));
        return err;
    }
...
kerr_t file_open_path_str(const char *path_str, file_t *file) {
    ...
    vnode_t *node;
    const kerr_t err = vnode_resolve_path_str(path_str, &node);
    if (err != KERR_NONE) {
        LOG_ERROR("failed to resolve path '%s', error %d (%s)", path_str, err,
                  kerr_str(err));
        return err;
    }
    return file_open_node(node, file);
...
kerr_t file_read(file_t *file, void *buf, size_t num_bytes, size_t *out_read) {
    ...
    if (err != KERR_NONE) {
        LOG_ERROR("node %p read returned %d: %s", node, err, kerr_str(err));
    }
    return err;
}
...
kerr_t file_write(file_t *file, const void *buf, size_t num_bytes,
    ...
    const kerr_t err = node->ops->f_write(node, (size_t)file->offset, buf,
                                          num_bytes, out_written);
