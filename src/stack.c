...
void
stack_new (stack_t * p_stack, void * p_bottom, size_t size_bytes)
{
    if (0 == size_bytes)
    {
        printf("stack_new: refuse to create an empty stack\n");
...
void
stack_push (stack_t * p_stack, uint32_t value)
{
...
bool
stack_is_full (stack_t const * p_stack)
{
    check_stack(p_stack);
...
bool
stack_is_empty (stack_t const * p_stack)
{
    check_stack(p_stack);
...
static void
check_stack (stack_t const * p_stack)
{
    if (!p_stack)
    {
        printf("check_stack: p_stack is NULL\n");
        panic("unexpected behavior");
    }

    if (p_stack->p_top_max < p_stack->p_bottom)
    {
        printf("check_stack: top is below bottom\n");
        panic("invalid stack");
    }
}
