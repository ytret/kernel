...
context_get () { eval "printf '%s' \"\${ctx_${1}${depth}}\""; }
...
context_pop () {
    if [ "$requireplan" = yes ] && [ "$(context_get count)" -gt 0 ] && [ -z "$(context_get plan)" ]
    then
    ...
    then
	ship_line "Expected $(context_get expect) tests but only $(context_get count) ran."
	status=1
    elif [ "$(context_get plan)" != "" ] && [ "$(context_get expect)" -lt "$(context_get count)" ]
    then
	ship_line "$(context_get count) ran but $(context_get expect) expected."
	status=1
    fi
}
...
do
    ...
    # YAML blocks are data, not structure; ignore indentation inside them.
    if [ "${state}" = "yaml" ]
    then
	ship_line "$line"
	if [ "$tok1" = "..." ]
